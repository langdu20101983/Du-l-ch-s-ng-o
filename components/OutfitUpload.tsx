...
export const OutfitUpload: React.FC<OutfitUploadProps> = ({ images, onAddImage, onRemoveImage, t }) => {
  ...
  return (
    <div className="space-y-4">
      <label className="block text-sm font-semibold text-gray-700">{t.outfitLabel}</label>
      <div className="flex flex-wrap gap-3">
        {images.map((img, idx) => (
          <div key={idx} className="relative group w-20 h-20">
            <img src={img} alt={`Outfit ${idx}`} className="w-full h-full object-cover rounded-xl border-2 border-indigo-100 shadow-sm" />
            <button
        ...
        >
          <i className="fa-solid fa-shirt text-xl mb-1"></i>
      ...
      <input
        ...
        accept="image/*"
      />
      <p className="text-xs text-gray-500">{t.outfitDesc}</p>
    </div>
  );
};
